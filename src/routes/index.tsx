...
import { Plus, TrendingUp, Clock, BookOpen, ChevronRight } from "lucide-react";
      ...
      <section className="border-b border-border bg-grain">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-8 sm:py-16">
          ...
          <div className="mt-10 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-4">
            ...
            <Stat label="Completed" value={`${completed} · ${completionPct}%`} icon={TrendingUp} />
            <Stat label="This week" value={formatDuration(weekSeconds)} icon={Clock} />
            <Stat label="Categories" value={String(categories.length)} icon={ChevronRight} />
          </div>
        </div>
      </section>
          ...
          {categories.map((c) => {
            ...
            return (
              <Link
                ...
                to="/category/$id"
                params={{ id: c.id }}
                className="group relative overflow-hidden rounded-xl border border-border bg-card p-5 transition-all hover:shadow-lift"
              >
                <div
